...
export default function ProductPageClient() {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-600 font-medium">
                  {product.rating || 4.9} {t(`(${product.reviews} 件のレビュー)`, `(${product.reviews} reviews)`, `(${product.reviews} 条评价)`)}
                </span>
      ...
      {product.aplusContent && product.aplusContent.map((imgUrl: string, idx: number) => (
        <section key={idx} className={`py-16 px-4 sm:px-6 lg:px-8 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
          <div className="max-w-7xl mx-auto">
      ...
      {/* Product Videos Section - Only shown if product has video content */}
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <div className="relative bg-black rounded-lg overflow-hidden aspect-video">
                  <img
                    src={productImages[0] || '/product-1.jpg'}
                    alt={t('商品動画', 'Product Video', '商品视频')}
                    className="w-full h-full object-cover"
                  />
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-white text-sm font-medium">{t('レビュー', 'Review', '测评')}</p>
                      <p className="text-white/80 text-xs">
                        {t(`${product.name} - 商品紹介`, `${product.name} - Product Overview`, `${product.name} - 商品介绍`)}
                      </p>
                    </div>
                  </div>
                  {/* Video Controls */}
              ...
              {/* Video List */}
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">{t('関連動画', 'Videos for this product', '相关视频')}</h3>
                <div className="space-y-3">
                  {[
                    {
                      title: t(`${product.name} - 詳細レビュー`, `${product.name} - Full Review`, `${product.name} - 深度测评`),
                      author: t('レビュー担当', 'Reviewer', '评测作者'),
                      duration: '2:26',
                      current: true,
                      badge: null
                    },
                    {
                      title: t(`${product.name} のセットアップ方法`, `How to Setup ${product.name}`, `${product.name} 的设置方法`),
                      author: t('販売者', 'Seller', '卖家'),
                      duration: '1:20',
                      current: false,
                      badge: t('販売者動画', 'Seller Video', '卖家视频')
                    },
                    {
                      title: t(`${product.name} - 開封レビュー`, `${product.name} - Unboxing & First Look`, `${product.name} - 开箱初体验`),
                      author: t('テックレビュー', 'TechReviewer', '科技评测'),
                      duration: '2:56',
                      current: false,
                      badge: null
                    },
                    {
                      title: t(`${product.name} の実機テスト`, `Testing ${product.name}, does it work?`, `${product.name} 实测效果如何？`),
                      author: t('デイリーテスター', 'Daily Tester', '日常测试'),
                      duration: '3:31',
                      current: false,
                      badge: null
                    },
                    ...
                    <div
                      key={index}
                      className={`p-3 rounded-lg cursor-pointer transition-all ${video.current
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="relative flex-shrink-0">
                          <img
                            src={productImages[index] || productImages[0] || '/product-1.jpg'}
                            alt={video.title}
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
              ...
              <div className="flex items-center gap-2 mb-4">
                <div className="flex">
                  {[...Array(5)].map((_, i: number) => (
                    <Star key={i} className={`w-6 h-6 ${i < Math.floor(product.rating || 4.9) ? 'text-orange-400 fill-current' : 'text-gray-300 fill-current'}`} />
                  ))}
                </div>
                <span className="text-sm text-gray-600">
                  {t(`${product.rating || 4.9} / 5`, `${product.rating || 4.9} out of 5`, `${product.rating || 4.9} / 5`)}
                </span>
              </div>
              ...
              <div className="space-y-2 mb-6">
                {[
                  { stars: 5, percent: Math.round((product.rating || 4.9) >= 4.5 ? 85 : 60) },
                  { stars: 4, percent: Math.round((product.rating || 4.9) >= 4.5 ? 10 : 25) },
                  { stars: 3, percent: Math.round((product.rating || 4.9) >= 4.5 ? 3 : 10) },
                  { stars: 2, percent: Math.round((product.rating || 4.9) >= 4.5 ? 1 : 3) },
                  { stars: 1, percent: Math.round((product.rating || 4.9) >= 4.5 ? 1 : 2) },
